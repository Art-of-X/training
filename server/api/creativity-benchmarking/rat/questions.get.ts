...
export default defineEventHandler(async (event) => {
    ...
    try {
        // Get all RAT questions (standardized test battery)
        const data = await readJsonData('rat-questions.json');
        const questions = data.questions || data; // Handle both formats
        
        // Get questions already answered by this user
        const answeredQuestions = await prisma.rATAnswer.findMany({
        ...
        console.error('Error getting RAT question:', error);
        throw createError({ statusCode: 500, message: 'Failed to get RAT question' });
    }
}); 