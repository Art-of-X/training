...
export async function generateAICoreResponse(
    ...
    *   **Diversify Questions (PRIORITY)**: If the conversation is becoming repetitive, shallow, or if the user's responses are brief and the conversation is not picking up naturally, **PRIORITIZE** using \`getPredefinedQuestion\` to fetch a high-quality, artist-centric question from the predefined list. Always integrate these questions naturally into the conversation flow.
