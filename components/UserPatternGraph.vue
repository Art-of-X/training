...
const hierarchyData = computed(() => {
  ...
  filteredPatterns.value.forEach(p => {
      ...
      if (!methodNode) {
          methodNode = { name: p.method, type: 'method', predefined: p.isPredefined, children: [] };
      ...
      if (p.competency) {
          ...
          if (!competencyNode) {
              competencyNode = { name: p.competency, type: 'competency', predefined: p.isPredefined, children: [] };
      ...
      if (p.spark) {
          const sparkNode: HierarchyNode = { 
              name: `ID_${String(p.id).padStart(3, '0')}`, // Use ID for the label
              type: 'spark', 
              content: p.spark, 
              predefined: p.isPredefined 
          };
