    ...
    <div class="container-wide flex flex-col flex-grow min-h-0">
      ...
      <div class="flex-grow items-start overflow-hidden min-h-0 py-8">
        ...
        <div class="h-full flex flex-col">
          <div
            class="sticky top-0 z-10 flex items-center justify-between border-b pb-2 pt-1 bg-white dark:bg-secondary-900 mb-4 flex-shrink-0"
          >
            <h1 class="text-2xl font-bold text-secondary-900 dark:text-white mb-2">
              Welcome,
              <span class="text-primary-500">{{ userProfile?.name || "Artist" }}</span
              >. Let's train your <span class="text-primary-500 italic">spark</span>.
            </h1>
          ...
          <div
            ...
            :class="{ 'overflow-y-auto': activeTab === 'static' }"
          >
                ...
                <div v-else class="block cursor-not-allowed opacity-60">
                  <div class="h-full">
                    <div class="flex items-center justify-between">
                      ...
                      <span
                        v-if="!module.enabled"
                        class="text-xs font-semibold bg-primary-100 text-primary-700 dark:bg-primary-900/50 dark:text-primary-300 px-2 py-1"
                        >SOON</span
                      >
                    </div>
                    <p class="text-sm text-secondary-600 dark:text-secondary-300 mt-1">
...
const loadSession = async (sessionMessages, sessionId) => {
  // First, switch to the chat tab to ensure the component is rendered
  activeTab.value = 'chat';

  // Wait for Vue to update the DOM and mount the ChatComponent
  await nextTick();

  // Normalize messages for the useChat composable
  ...
  // Now that the component is mounted, chatRef should be available
  if (chatRef.value?.loadSession) {
    ...
    console.error('[Dashboard] Error: chatRef or loadSession not available after tab switch.');
  }
};
...
onMounted(async () => {
  try {
    const [
      ...
      $fetch("/api/creativity-benchmarking/progress?type=aut").catch((e) => ({
        answered: 0,
        total: 0,
      })),
      $fetch("/api/creativity-benchmarking/progress?type=rat").catch((e) => ({
        answered: 0,
        total: 0,
      })),
      $fetch("/api/creativity-benchmarking/progress?type=dat").catch((e) => ({
        answered: 0,
        total: 0,
      })),
    ...
    progressData.value = {
      creativity: {
        completed:
          (autProgress.answered > 0 ? 1 : 0) +
          (ratProgress.answered > 0 ? 1 : 0) +
          (datProgress.answered > 0 ? 1 : 0),
