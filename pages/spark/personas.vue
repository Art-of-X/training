    ...
    <div v-else>
      <section class="border-b-4 border-primary-500 pb-4">
        <div class="flex flex-wrap items-center justify-between gap-4">
          <h1 class="text-3xl font-bold">Sparks</h1>

          <div class="flex items-center gap-2 justify-end">
            <input v-model="filters.nameKeyword" type="text" placeholder="Search by name..." class="form-input w-48" />
            ...
            <button v-if="hasActiveFilters" @click="clearFilters" class="btn-secondary">Clear</button>
          </div>
        </div>
      </section>
      ...
      <section>
        ...
        <div v-else-if="filteredSparks.length === 0 && hasActiveFilters" class="text-center py-12">
          <div class="text-secondary-500">
            ...
            <p class=" text-sm  font-medium mb-2">No sparks found</p>
            <p class=" text-sm ">
              Try adjusting your filters or
              <button @click="clearFilters" class="text-primary-500 hover:text-primary-600 underline">
                clear all filters
              </button>
            </p>
          </div>
